...
export function generateFloor(seed: number, floorNumber = 1): FloorState {
  ...
  if (objective.type === 'defeatSpecial' || objective.type === 'finalBoss') {
    ...
    enemies.push({
      id: nextId('enemy'),
      type: 'morbol',
      aiType: isBoss ? 'boss' : def.aiType,
      aiState: 'free',
      aggroRange: isBoss ? 8 : def.aggroRange,
      power,
      x: specialPos.x,
      y: specialPos.y,
      hp,
  ...
  return {
    ...
    rooms: layout.rooms.map<FloorRoomState>((room) => ({
      id: room.id,
      left: room.left,
      top: room.top,
      right: room.right,
    ...
    corridors: layout.corridors.map<FloorCorridorState>((corridor) => ({
      id: corridor.id,
